...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:mml="http://www.w3.org/1998/Math/MathML" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xlink="http://www.w3.org/1999/xlink" version="2.0" exclude-result-prefixes="xsi xs xlink mml">
	...
	<xsl:variable name="journalLinkID">
        <xsl:choose>
            ...
            <xsl:when test="contains(//div[@class='front']//*[@class='jrnlTitle'],'Physiology')">
                <xsl:text>physiology</xsl:text>
            </xsl:when>
            <xsl:when test="contains(//div[@class='front']//*[@class='jrnlTitle'],'Plant Science')">
                <xsl:text>Plant_Science</xsl:text>
            </xsl:when>
            <xsl:when test="contains(//div[@class='front']//*[@class='jrnlTitle'],'Frontiers in Genetics')">
                <xsl:text>genetics</xsl:text>
            </xsl:when>
            <xsl:when test="contains(//div[@class='front']//*[@class='jrnlTitle'],'Cardiovascular Medicine')">
    ...
    <xsl:template match="//p[@class='jrnlJVH']" priority="5">
        <p>
            <xsl:apply-templates select="@*"/>
    ...
    <xsl:template match="//p[@class='jrnlJT']/*[@class='jrnlTitle']" priority="5">
        <xsl:element name="{local-name()}">
            <xsl:apply-templates select="@*"/>
            <xsl:variable name="href1">
               <xsl:choose>
     ...
     <xsl:template match="//div[@class='jrnlPresentAddrFN']" priority="5">
        ...
        <div>
            ...
            <div class="jrnlPresentAddrFNPara">
             <!--   <p class="jrnlAuthor">
                    <xsl:apply-templates select="//div[@class='front']/p[@class='jrnlAuthors']//*[@class='jrnlAuthor'][following-sibling::*[@class='jrnlPresentAddrRef'][@data-rid=$refID]]/node()"/>
                </p> -->
