...
<!-- Moved this firstcitation template match to structure.xsl -->
   <!-- <xsl:template match="//span[@class='jrnlFigRef']|//span[@class='jrnlVidRef']|//span[@class='jrnlTblRef']|//span[@class='jrnlBoxRef']" priority="5">
    ...
    <xsl:choose>
        <xsl:when test="$journal ='singlecolumn'"/>
        <xsl:otherwise>
            <xsl:element name="{name(.)}">
                <xsl:apply-templates select="@*|node()"/>
            </xsl:element> 
  ...
  <!--  <xsl:template match="//div[@class='back']/p[last()]" priority="5">
	...
	<xsl:template match="*[@class='body']//*[@class='jrnlSecPara'][./preceding-sibling::label]" priority="5">
        <xsl:element name="{name()}">
            <xsl:apply-templates select="@*"/>
        ...
        <xsl:when test="./preceding-sibling::label[1]">
                ...
                <xsl:variable name="fs" select="generate-id(./following-sibling::*[contains(@class,'jrnlSecPara')][1])"/> 
	...
	<!--<xsl:template match="//table[@data-class='jrnlExampleTbl']//tr//td/p" priority="6">
        <xsl:element name="{name(.)}">
            <xsl:attribute name="class">
    ...
    <xsl:template match="*[@class='WordSection1']/div[@class='jrnlAppGroup']" priority="5">
        <xsl:variable name="blockId" select="count(preceding::*[@class='jrnlAppGroup']) + 1"/>
        <div class="jrnlAppGroup">
            <xsl:for-each select=".//*[@class='jrnlAppBlock']">
                <xsl:for-each select=".//table[@app-landscape='true']">
                    <xsl:variable name="pid" select="concat('BLK_A',$blockId,'-T',position())"/>
    					<div class="floatBlock" id="{$pid}" data-stream-name="{concat('a_A',$blockId,'-T',position())}" data-id="{concat('A',$blockId,'-T',position())}">
    						<div class="jrnlTblBlock" id="{$pid}" data-id="{concat('A',$blockId,'-T',position())}" data-stream-name="{concat('a_A',$blockId,'-T',position())}">
    						    <xsl:element name="{name(.)}">
                                    <xsl:apply-templates select="@*[name()!='app-landscape']"/>
        							<xsl:apply-templates select="node()"/>
    							</xsl:element>
    						</div>
    					</div>
	...
	<xsl:template match="*[@class='WordSection1']//div[@class='jrnlAppGroup']//*[@class='jrnlAppBlock']/p[following-sibling::*[1][@app-landscape='true']]" priority="7">
	<xsl:variable name="appcount" select="count(parent::*[@class='jrnlAppBlock']/parent::div[@class='jrnlAppGroup'][preceding::*[@class='jrnlAppGroup']]) + 1"/>
	<xsl:variable name="tblcount" select="count(preceding-sibling::table[@app-landscape='true']) + 1"/>
    	<xsl:element name="{name(.)}">
    	    <xsl:apply-templates select="@*|node()"/>
    	    <span class="jrnlFirstCitation" data-rid="{concat('A',$appcount,'-T',$tblcount)}"/>
        </xsl:element>
	</xsl:template>
    <xsl:template match="*[@class='WordSection1']//div[@class='jrnlAppGroup']//*[@class='jrnlAppBlock']//table[@app-landscape='true']" priority="8"/>
    <xsl:template match="*[@class='body']/label[following-sibling::*[contains(@class,'jrnlSecPara')][1]]" priority="8"/>
    ...
    <xsl:template match="//target[@target-type='page']" priority="5"/>
    <!--<xsl:choose>
