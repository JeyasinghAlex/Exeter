...
var floatPlacer = function() {
    ...
    config["currDocDOM"] = currBlockXMLItem;//config["currDoc"].xmlElements[0].evaluateXPathExpression(domXpath)[0];//appending document DOM object to config
    ...
    config["retriggeringFloatPlacerCt"] = 0;
    config["carryForwardFromPage2"] = false;
    config["pageFloatsInfo"] = [];
    config["pageColumnBaseAlignInfo"] = [];
    config["inlineTableContinuedUptoPage"] = '';
    config["inlineTableContinuedUptoPageID"] = '';
//collecting docs all citations as an array - starts
    ...
    if(retriggeringFloatPlacer){
        config['columnDetails'] = currRunOnArticleObj.layoutDetails.columnDetails.columnDetails;
        config['openerPageColumnDetails'] = currRunOnArticleObj.layoutDetails.columnDetails.openerPageColumnDetails;
        }
    else {
    ...
    var floatsPerUserInputsOverride = config.currDocDOM.evaluateXPathExpression("//div[@data-override-page]");
    var floatsPerUserInputs = config.currDocDOM.evaluateXPathExpression("//div[@data-override-page]");
    ...
    for (var fpu = 0; fpu < floatsPerUserInputsLen; fpu ++){
        try{
            ...
            var currUserFltIDPage = floatsPerUserInputs[fpu].xmlAttributes.itemByName("data-override-page").value;
        ...
        floatPlacer.events = {
            collectCitation: function (){
                //if (config.totalCitationLength > 0){//commented on 11-Oct-2016 because for basealign part should execute though there is not floats
                    ...
                    if (floatPlacerLastProcessedPage !=  '' && (retriggeringFloatPlacer) && (config.retriggeringFloatPlacerCt == 0)){
                        currPage = floatPlacerLastProcessedPage;
                        config.retriggeringFloatPlacerCt = 1;
                        }
                    ...
                    floatPlacerLastProcessedPage = currPage;
                    displayMSG("Placing floats and base aligning page: " + String(currPage.name) + "\tin-progress\t" + Math.round(currProgressPercent));
                    debuggerMSG("Placing floats and base aligning page: " + String(currPage.name));
                        ...
                        var currPageFrame;
                        app.activeDocument.recompose();
                        //assigning margin details
                        if(retriggeringFloatPlacer){
                            config["margin"] = currRunOnArticleObj.layoutDetails.columnDetails.otherPageMargin;
                            if (currPageName == 1){
                        ...
                        for (var pfc = 0; pfc < currPageFrameParasLen; pfc ++){
                            ...
                            if ((typeof(currPageFrameParas[pfc]) != 'undefined') && currPageFrameParas[pfc].isValid && !(currPageFrameParas[pfc].associatedXMLElements.length > 1)){//if this condition is true then assuming that correct associated xml element was not located
...
//~                                         }
                            ...
                            else if (currPageFrameParas[pfc].isValid && currPageFrameParas[pfc].associatedXMLElements.length > 1){
                                                ...
                                                for (var hzc = 0; hzc < currProcessClmsLen; hzc ++){
                                                    ...
                                                    if (currCitStHrOffset < currClmnWdExtrmHrOffset){
                                                        ...
                                                        if (citationIndex == 0 && !(prevCitClmIndex < currCitClmIndex)){//the very first citation will go through this case
                                                            ...
                                                            if (respectiveFloat[0].xmlAttributes.itemByName("data-override-page").isValid){
                                                                if (respectiveFloat[0].xmlAttributes.itemByName("data-override-page").value == 1){
                        ...
                        if(!(config.userPlacingFloatsDetails[currPage.name] == null)){
                            ...
                            function processFirstPageBlocks(){
                                ...
                                for(var fpc = 0; fpc < citationBeingMovedLen; fpc++){                              
                                    ...
                                    config.carryForwardFromPage2 = true;
                                    citation(currPageCitID, currCitBlkArea, currPageCit, defaultFitOnPage, forceMove, totalCitHt, currCitBlkHt, hzc, currPageName, currCitBaseline);
                                    config.carryForwardFromPage2 = false;
...
//~                             }//end of 
                    ...
                    ResolveOversetInFloatPlacer(app.activeDocument);
                    app.activeDocument.recompose();
                    if (config.notPlacedFloatsIDs.length > 0|| app.activeDocument.pages[String(currPageName)].isValid || config.citationBeingMoved.length > 0){//checking whether "notPlacedFloatsIDs" are avaible or moving to next page for citation check
                        if (!(app.activeDocument.pages[String(currPageName)].isValid)){//if this condition is true the float will placed on next empty pages
                            ...
                            var myMasterPage = nextPage.appliedMaster;
                            if (nextPage.side == PageSideOptions.leftHand)
                              {
                                    ...
                                    var tFramesLen = tFrames.length;
                                        ...
                                        var fLabel = tFrames[f].label;
                                            if (fLabel == 'VERSO'){
                                            var currFrameFloatPlacer = tFrames[f];
                                            f = tFramesLen;
                                            }
                                    }    
                                currFrameFloatPlacer.override(nextPage);
                                app.activeDocument.recompose();
                                }//end of IF
                            else{
    ...
    //                        tempFigureFrm.geometricBounds = [tempFigureFrm.geometricBounds[0], tempFigureFrm.geometricBounds[1] + config.pageSize.width, tempFigureFrm.geometricBounds[2], tempFigureFrm.geometricBounds[1] + config.pageSize.width +config.textFrmColWidth];
                            ...
                            config["placeUptoImaginaryColumn"] = false;//a boolean flag set for the current float has been extended to place upto imaginary column(s)
                            ...
                            if (respectiveFloat[0].xmlAttributes.itemByName("data-column-span").isValid){
                                userInputSpanTo = respectiveFloat[0].xmlAttributes.itemByName("data-column-span").value;
                                userInputSpanTo = parseInt(userInputSpanTo);
                                }
                            //===================================================
                            if(currPage.name == '1'){
                            ...
                            else if (String(userInputSpanTo) != ''){
                                imageWidth = [currProcessClmsWidth[userInputSpanTo][0].width]
                                }
                            var maxWidth = currProcessClmsWidth[currProcessClmsWidthLen -1][0].width;
                            var maxSingleColWidth = currProcessClmsWidth[0][0].width;
                            //========================================================================
                            //below code update was done by Augustine K on 04-Jul-2017
                            //update: based on the image resize percent input from user resizing the image.
                            //now calculating user resizing percentage
                            var floatPercent = 100;//assuming the default value would be null
                            if (respectiveFloat[0].xmlAttributes.itemByName("data-width-factor").isValid){
                                floatPercent = respectiveFloat[0].xmlAttributes.itemByName("data-width-factor").value;
                                floatPercent = Math.max(65,parseInt(floatPercent));//the minimum percent from the user should be 75 anything lesser than that will be considered as 75, the min downsize limit has been changed to 65 on 02-Aug-2017 
                                if (imageWidth == 0){
                                    maxWidth = (maxWidth * floatPercent)/100
                                    maxSingleColWidth = (maxSingleColWidth * floatPercent)/100                                     
                                    }
                                else {
                                    imageWidth = (imageWidth * floatPercent)/100
                                    }
                                //some case image may have a border with some indent, so if the config satifies that condition then excluding the indent from maxWidth, maxSingleColWidth and imageWidth
                                //code update done on 20-Oct-2017
                                if ((typeof(config.resizeImage.floatImageBorder) != 'undefined')){
                                    maxWidth = maxWidth - config.resizeImage.floatImageBorder.insetValue;
                                    maxSingleColWidth = maxSingleColWidth - config.resizeImage.floatImageBorder.insetValue;
                                    imageWidth = imageWidth - config.resizeImage.floatImageBorder.insetValue;
                                    }//end of IF
                                }
                            //========================================================================
                            ...
                            if (floatOrientation == 'landscape'){
                                ...
                                var imageData = {
                                    "maxWidth": maxWidth,
                                    "maxSingleColWidth": maxSingleColWidth,
...
//~                             logTimer("Updating glyphs...");       
                            ...
                            if (tempFigureFrm.allGraphics.length == 0 && !(tempFigureFrm.tables[0].isValid) && tempFigureFrm.rectangles.length == 0 && floatClass != 'jrnlDigestBlock'){
                                var floatBlockXML = respectiveFloat[0];
                                if (floatClass == 'jrnlFigBlockGroup'){
                                    var floatBlockXMLElements = floatBlockXML.xmlItems[0].xmlElements;
...
//~                                     var floatBlockCurrXMLParaStyleName = floatBlockCurrXMLPara.appliedParagraphStyle.name;
                            ...
                            config.currDoc.viewPreferences.rulerOrigin = RulerOrigin.pageOrigin;
                            //=====================================
                            config["screenBackgroundShade"] = true;//checking whether the frame background colr needs to be removed based on user input
                            if (respectiveFloat[0].xmlAttributes.itemByName("data-p-shade").isValid){
                                if (respectiveFloat[0].xmlAttributes.itemByName("data-p-shade").value == 'false'){
                                    config.screenBackgroundShade =  false
                                    }//end of IF
                                }//end of IF
                            //=====================================
                            if (tempFigureFrm.allGraphics.length > 0 && !(tempFigureFrm.tables[0].isValid)){
                               ...
                               //now removing the background shade for frame if user input says to remove
                               if (!config.screenBackgroundShade){
                                   tempFigureFrm.fillColor = config.currDoc.swatches[0];
                                   }
                                }//end of IF
                            else if (tempFigureFrm.tables[0].isValid){
                                _table_colspan_stroke(tempFigureFrm, 4)//applying table span rule
                                tempFigureFrm.appliedObjectStyle = config.currDoc.objectStyles.itemByName('TBL');
                                //the below code will fit the height for cells=========
                                var cellsWithRowHeight = tempFigureFrm.tables[0].associatedXMLElement.evaluateXPathExpression('//Cell[@data-height]');
                                var cellsWithRowHeightLen = cellsWithRowHeight.length;
                                for (var rht = 0; rht < cellsWithRowHeightLen; rht ++){
                                    try{
                                        var currCellHeightValue = cellsWithRowHeight[rht].xmlAttributes.itemByName('data-height').value;
                                        cellsWithRowHeight[rht].cells[0].height = parseInt(currCellHeightValue);
                                        }catch(e){}
                                   }
                                //========================================
                                //applying border for image if the config allows, code updated on 20-Oct-2017
                                if ((typeof(config.resizeImage.floatImageBorder) != 'undefined') && (tempFigureFrm.allGraphics.length > 0)){
                                    var imageParentRectFrame = tempFigureFrm.allGraphics[0].parent;
                                    var imageParentRectFrameBounds = imageParentRectFrame.geometricbounds;
                                    imageParentRectFrame.appliedObjectStyle = config.currDoc.objectStyles.itemByName('FIG_BORDER');
                                    imageParentRectFrame.geometricBounds = [imageParentRectFrameBounds[0],imageParentRectFrameBounds[1],imageParentRectFrameBounds[2] + config.resizeImage.floatImageBorder.insetValue,imageParentRectFrameBounds[3]+config.resizeImage.floatImageBorder.insetValue]
                                    }
                               //now removing the background shade for frame if user input says to remove
                               if (!config.screenBackgroundShade){
                                   tempFigureFrm.fillColor = config.currDoc.swatches[0];
...
//                            var tempFigureFrmLastParaBasline = tempFigureFrm.paragraphs.lastItem().lines.lastItem().baseline;
                                ...
                                if (!(currProcessClmsWidth[sp-1] == undefined)){
                                    ...
                                    if (parseInt(currFloatWd*100)/100 > parseInt(preSpnWdith*100)/100 && parseInt(currFloatWd*100)/100 <= parseInt(currSpnWdith*100)/100){
                                        ...
                                        if (config.landscapeFloatOnFirstPage == false && currPage.name == 1){
                                            ...
                                            //checking further whether the config allows any miscColumns for float placement
                                            //if so then checking whether the float width is lesser or equal after including the misc column's width
                                            if(typeof(config.pageColumnDetails[currProofColumnType].miscColumnDetails) != 'undefined'){
                                                if ((parseInt(currFloatWd*100)/100) <= ((config.pageColumnDetails[currProofColumnType].miscColumnDetails[0].width + config.pageColumnDetails[currProofColumnType].miscColumnDetails[0].gutter + currSpnWdith)*100/100)){
                                                    config.rotateFloat = false;
                                                    config.placeUptoImaginaryColumn = true;
                                                    }
                                                }
                                             }
                                        if (!(tempFigureFrm.overflows) && (tempFigureFrmTempHt >currSpnWdith) && !(config.placeUptoImaginaryColumn)){
                            ...
                            if (!config.rotateFloat){
                               ...
                               var tempHt = currFloatWd;
                                ...
                                if (config.landscape.singleColumnStyle && (wdAfterRotate <= singleClmMaxWidth)){//if this condition is true then the float could be placed in single colmn
                                    ...
                                    //further checking whether the config has to property misc columns, if so then excluding that from the wdDiff
                                    if (typeof(config.pageColumnDetails[currProofColumnType].miscColumnDetails) != 'undefined'){
                                        wdDiff = (currFloatHt + config.pageColumnDetails[currProofColumnType].miscColumnDetails[0].width + config.pageColumnDetails[currProofColumnType].miscColumnDetails[0].gutter) - (wdAfterRotate); 
                                        config.placeUptoImaginaryColumn = true;//assuming then the float placement also includes miscColumns
                                        }
                                    else {
                            ...
                            if (tempFigureFrm.overflows){
                                ...
                                if (tempFigureFrm.allGraphics.length > 0 && !(tempFigureFrm.tables[0].isValid)){
                                    config["figureContinuedBlockItteration"] = 0;
                                    config.continuedBoxDetails = [];
                                    config.continuedBoxID = [];
                                    config["firstFigureFrameInfo"];
                                    figureContinuedBlock(tempFigureFrm, currPageName, boundsForContFrms, currCitation, hzc, floatSpnTo, currCitBaseline, config.rotateFloat, wdSplitEqually, defaultFloatContCt, currCitBlkHt, 'FIG',config,currProcessClms);
                                    }//end of IF
                                else {
                                    floatContinuedBlock(tempFigureFrm, currPageName, "TBL_CONT_BOTTOM", "jrnlTblCaption", "TBL_CONT_TOP", tableFrameCurr, true, currCitation, hzc, floatSpnTo, currCitBaseline, config.rotateFloat, wdSplitEqually, defaultFloatContCt, currCitBlkHt, boundsForContFrms, 'TBL');
                                    }//end of ELSE
                                }
                            else {
                                ...
                                //cheking whether the float width span to imaginary column width, if so the frame width also includes that column
                                if (config.placeUptoImaginaryColumn && !(config.rotateFloat)){
                                    if (currPage.name == 1){
                                        tempFigureFrm.geometricBounds = [tempFigureFrm.geometricBounds[0], tempFigureFrm.geometricBounds[1], tempFigureFrmLastParaBasline + tempFigureFrmBtmInset, config.openerPageColumnsSpanWidth[floatSpnTo][0].width + tempFigureFrm.geometricBounds[1] + config.pageColumnDetails[currProofColumnType].miscColumnDetails[0].width  + config.pageColumnDetails[currProofColumnType].miscColumnDetails[0].gutter];                                            
                                        }//end of IF
                                    else {
                                        tempFigureFrm.geometricBounds = [tempFigureFrm.geometricBounds[0], tempFigureFrm.geometricBounds[1], tempFigureFrmLastParaBasline + tempFigureFrmBtmInset, config.columnSpanWidth[floatSpnTo][0].width + tempFigureFrm.geometricBounds[1] + config.pageColumnDetails[currProofColumnType].miscColumnDetails[0].width + config.pageColumnDetails[currProofColumnType].miscColumnDetails[0].gutter];
                                        }//end of ELSE
                                    }//end of IF
                                //=====================================================================
                                //cheking whether the figure has side caption property, if so trying to move caption to the side
                                if (respectiveFloat[0].xmlAttributes.itemByName("data-side-caption").isValid){
                                    var return_value = '';
                                    var currWrapInfo = [wrapTop, wrapLeft, wrapBottom, wrapRight];
                                    return_value = _side_caption(tempFigureFrmID, floatBlockHt,return_value,currWrapInfo);
                                    tempFigureFrmID = return_value[0];
                                    floatBlockHt = return_value[1] + wrapBottom;
                                    floatBlockArea = floatBlockHt * floatBlockWd;//Total float block area including caption
                                    }//end of IF (side caption)
                            ...
                            config.placeUptoImaginaryColumn = false;
                            config.continuedBox = false;
                            config.continuedBoxDetails = [];
                            config.continuedBoxID = [];
                            if (respectiveFloat[0].xmlAttributes.itemByName("data-top-gap").isValid){
                            ...
                            //=============================================================================
                            return floatSpnTo, floatBlockWd, floatBlockHt, floatBlockArea, tempFigureFrmID, config.rotateFloat, wdSplitEqually, floatContinued, config.continuedFloatDetails, wrapTop, wrapBottom, config.currFloatClassName;
                            }    
                        }//end of IF
                        ...
                        config.preferredOnColumn = '';
                        config.preferredOnCurrentPage = '';
                        config.currFloatClassName = config.docFloatIDsArray[currCitID].currFloatClassName;
                        //config.notPlacedFloatsIDs.unshift(currCitID);
                        return floatSpnTo, floatBlockWd, floatBlockHt, floatBlockArea, tempFigureFrmID, config.rotateFloat, wdSplitEqually, wrapTop, wrapBottom, config.currFloatClassName;
            ...
            placingSandwich: function(pageFloatIDsArray,currPage,placeStyle){
                ...
                if (firstFrame.isValid){
                    if (firstFrame.paragraphs.length > 0){
                        ...
                        if (firstFrame.paragraphs.length == 1){
                            var nextParaPageName = currFrmLastParaPageName;
                            }
                        else {
                ...
                //if the page has imaginary column then the top bound and columns last line base would be equal to the current first columns values, so add the value using Array.unshift method(earleir we forcely checked whether the first column has imaginary column but now checking each and every columns)
                for (var iclm = 0; iclm < currProcessClmsLen; iclm ++){
                    if(currProcessClms[iclm].imaginary == true){
                ...
                for (var cw = 0; cw < pageFloatIDsArrayLen; cw ++){
                    var currFltClassName = config.docFloatIDsArray[pageFloatIDsArray[cw]].currFloatClassName;
                    ...
                    if(currFltClassName != 'jrnlTblBlock'){
...
//~                         currBlockItem.select();
               ...
               function calculatingFloatsPlacementIndex(flt, pageClmDetails, pageFloatIDsArrayLen){
                    for (flt; flt < pageFloatIDsArrayLen; flt ++){
                        ...
                        if (currPage.name == '1'){
                            //the below condtion has been changed on 16-Jul-2017, the earlier condition by default won't allow any float on first page first column top
                            //but now that has been changed so if user force to place float of frist column top it will allow
                            var currFloatXML = config.currDocDOM.evaluateXPathExpression("//div[@id='BLK_"+currFloatID+"']");
                            if (currFloatXML[0].xmlAttributes.itemByName("data-position").isValid && currFloatXML[0].xmlAttributes.itemByName("data-column-start").isValid){
                                var currFloatXMLPlaceAt = currFloatXML[0].xmlAttributes.itemByName("data-position").value;
                                var currFloatXMLColumnStart = parseInt(currFloatXML[0].xmlAttributes.itemByName("data-column-start").value);
                                if (!(currFloatXMLPlaceAt == 'top' && currFloatXMLColumnStart == 0)){
...
//~                                     else if ((bsp + 1) < pageFloatIDsArrayLen){
                        ...
                        config["forceUseUserPosition"] = false;
                        config["forceUseUserColumnStart"] = false;
                        ...
                        if (config.currDocDOM.evaluateXPathExpression("//div[@id='BLK_"+currFloatID+"']").length > 0){
                            if (currFloatXML[0].xmlAttributes.itemByName("data-position").isValid){
                                var currFloatXMLPlaceAt = currFloatXML[0].xmlAttributes.itemByName("data-position").value;
                                    userInputPlaceFloatAt = currFloatXMLPlaceAt;
                                    config.forceUseUserPosition = true;
                                }//end of IF
                            if (currFloatXML[0].xmlAttributes.itemByName("data-column-start").isValid){
                                    userInputPlaceFloatAtClm = parseInt(currFloatXML[0].xmlAttributes.itemByName("data-column-start").value);                                
                                    config.forceUseUserColumnStart = true;
                                }//end of IF
                            }//end of IF
                        debuggerMSG("On page num " +currPage.name +", "+ currFloatID +", "+ placeFloatAt +", "+ placeFloatAtClm +", "+ flt);
                        if (placeFloatAt != ''){
                            if (config.forceUseUserPosition && config.forceUseUserColumnStart){
                                ...
                                config.docFloatIDsArray[currFloatID]["userInput"] = true;                                
                                config.docFloatIDsArray[currFloatID]["forceUseUserPosition"] = true;                                
                                config.docFloatIDsArray[currFloatID]["forceUseUserColumnStart"] = true;                                
                                }
                            else if (config.forceUseUserPosition && !(config.forceUseUserColumnStart)){
                                config.docFloatIDsArray[currFloatID].floatPlaceFrom = userInputPlaceFloatAt;
                                config.docFloatIDsArray[currFloatID]["placeFloatAtClmIndex"] = placeFloatAtClm;
                                config.docFloatIDsArray[currFloatID]["userInput"] = false;                                
                                config.docFloatIDsArray[currFloatID]["forceUseUserPosition"] = true;                                
                                config.docFloatIDsArray[currFloatID]["forceUseUserColumnStart"] = false;                                
                                }
                            else if (!(config.forceUseUserPosition) && (config.forceUseUserColumnStart)){
                                config.docFloatIDsArray[currFloatID].floatPlaceFrom = placeFloatAt;
                                config.docFloatIDsArray[currFloatID]["placeFloatAtClmIndex"] = userInputPlaceFloatAtClm;
                                config.docFloatIDsArray[currFloatID]["userInput"] = false;                                
                                config.docFloatIDsArray[currFloatID]["forceUseUserPosition"] = false;                                
                                config.docFloatIDsArray[currFloatID]["forceUseUserColumnStart"] = true;                                
                                }
                            else {
                                config.docFloatIDsArray[currFloatID].floatPlaceFrom = placeFloatAt;
                ...
                if (config.placeStyle == 'Sandwich') {//else the style would be 'sandwich'
                    ...
                    for (var cflt = 0; cflt < currFloatsForCurrPageLen; cflt ++){
                        ...
                        //==============================================================================================================
                        if(spanValue <= nextAvblClms || (config.docFloatIDsArray[currFloatID].forceUseUserPosition && config.docFloatIDsArray[currFloatID].forceUseUserColumnStart)){
                            var floatSpanToClms = spanValue+clmIndex;//the actual number of columns to which float will span to
                            ...
                            if (heightFits){
                                for(var chkHt = floatSpanToClms; chkHt >= clmIndex; chkHt --){
                                    if (currFloat.forceUseUserPosition){
                                        floatPositionAt = currFloat.floatPlaceFrom;
                                        flagAtPositioningNode = true;
                                        }
                                    else if (config.floatPositionFlagArray[chkHt][floatPositionAt] == 'OPEN' && !(currFloatPlaceInline)){
                                        flagAtPositioningNode = true;
                                        }//end of IF
                                        ...
                                        if (floatPositionAt == 'top' && config.floatPositionFlagArray[chkHt].bottom == 'OPEN'){
                                            ...
                                            if (clmsAvblToRight > spanValue + 1){//then checking if that many clms are avilable to place this float beyond the current clm
                                                ...
                                                if (config.docFloatIDsArray[currFloatID].userInput && !(config.docFloatIDsArray[currFloatID].placingInline)){//have identified a bug in the code, when the float is at the last page earlier code skips user input, now code has been changed to prioritize user input
                                                    currFloat.placeFloatAtClmIndex = config.docFloatIDsArray[currFloatID].placeFloatAtClmIndex;                                                
                                                    flagAtPositioningNode = true;
                                                    config.floatPlaceInline = false;
                                                    clmIndex = config.docFloatIDsArray[currFloatID].placeFloatAtClmIndex;
                                                    chkHt = clmIndex;
                                                    floatSpanToClms = clmIndex;
                                                    }
                                                else {
   ...
   //                                                 }//end of IF
                ...
                if (config.floatPlaceInline){
                    ...
                    for (var plcInlFlt = 0; plcInlFlt < currentPageInlineFloatsLen; plcInlFlt ++){
                        ...
                        var citXMLNodeLen = citXMLNode.length;
                        for (var fltCit = citXMLNodeLen - 1; fltCit >= 0; fltCit --){
                            var currInlFltCit = citXMLNode[fltCit];
                            var currInlFltCitAttrib = currInlFltCit.xmlAttributes;
                            var currInlFltCitAttribLen = currInlFltCitAttrib.length;
                            for (var citAtt = 0; citAtt < currInlFltCitAttribLen; citAtt ++){
                                if(currInlFltCitAttrib[citAtt].value == "jrnlFirstCitation"){
                                    fltCit  = -1;//if the above condition is true then stopping the previous loop
                                    ...
                                    if (config.pageFloatsInfo == ''){
                                        floatsPlacementjson.write("\'BLK_" + String(currFltPlaceInl) + "\':\'" + currPage.name + "\'");
                                        }
                                    else {
                                        floatsPlacementjson.write(",\'BLK_" + String(currFltPlaceInl) + "\':\'" + currPage.name + "\'");
                                        }
                                    ...
                                    app.paste();
                                    config.floatsPlacedOnPages[config.floatsPlacedOnPages.length] = config.currentPageInlineFloats[plcInlFlt];//Placed floats IDs were stored in an separate array
                                    currInlFltCitAttrib[citAtt].value = "jrnlFirstCitationPlaced";
                                    //storing the float placed on page as an object
                                    if (config.pageFloatsInfo[currPage.name] == null) {
                                        config.pageFloatsInfo[currPage.name] = "BLK_"+ currFltPlaceInl;
                                        }
                                    else {
                                        config.pageFloatsInfo[parseInt(currPage.name)] = config.pageFloatsInfo[parseInt(currPage.name)].concat("BLK_"+ currFltPlaceInl);
                                        }
                                    //=======================================
                                    ...
                                    //then removing that particular ID out from config.notPlacedFloatsIDs//this part has been added on 07-Jul-2017 by Augustine K
                                    var notPlacedFloatsIDslen = config.notPlacedFloatsIDs.length;
                                    for(var fInp =0; fInp < notPlacedFloatsIDslen; fInp ++){
                                        var currObj = config.notPlacedFloatsIDs[fInp];
                                        if (currFltPlaceInl == currObj){
                                            config.notPlacedFloatsIDs.splice(fIDrem,1);
                                            break;
                                            }//end of IF
                                        }//end of FOR loop
                ...
                for (var ftm = 0; ftm < floatsToBeLocatedLen; ftm ++){
                    ...
                    floatsPlacementjson.open('a');
                    if (config.pageFloatsInfo == ''){
                        floatsPlacementjson.write("\'BLK_" + String(floatsToBeLocated[ftm]) + "\':\'" + currPage.name + "\'");
                        }
                    else {
                        floatsPlacementjson.write(",\'BLK_" + String(floatsToBeLocated[ftm]) + "\':\'" + currPage.name + "\'");
                        }
                    floatsPlacementjson.close();
                    var currFloatToBeLocated = config.docFloatIDsArray[floatsToBeLocated[ftm]];
//~                     var currFloatSpanToImagClm = false;
//~                     if (typeof(currFloatToBeLocated.spanUptoImaginaryColumn) != 'undefined'){
//~                         currFloatSpanToImagClm = true;
//~                         }
                    config.currDoc.recompose();
                    config.currDoc.pageItems.itemByID(currFloatToBeLocated.frameID).move(currPage);
                    if (currFloatToBeLocated.floatRotated){
                        if (currFloatToBeLocated.spanUptoImaginaryColumn){//code here has been changed on 14-Oct-2017, the rule here if the float has span width extends to imaginary then offset the img col width with actual position
                            config.currDoc.pageItems.itemByID(currFloatToBeLocated.frameID).move([currFloatToBeLocated.floatBounds[0] + currFloatToBeLocated.landscapeAdditionalOffset - ((config.pageColumnDetails[currProofColumnType].miscColumnDetails[0].width  + config.pageColumnDetails[currProofColumnType].miscColumnDetails[0].gutter)), config.pageColumns[0].bounds[2]]);
                            }
                        else {
                        ...
                        if (currFloatToBeLocated.spanUptoImaginaryColumn){//code here has been changed on 14-Oct-2017, the rule here if the float has span width extends to imaginary then offset the img col width with actual position
                            config.currDoc.pageItems.itemByID(currFloatToBeLocated.frameID).move([currFloatToBeLocated.floatBounds[0] - (config.pageColumnDetails[currProofColumnType].miscColumnDetails[0].width  + config.pageColumnDetails[currProofColumnType].miscColumnDetails[0].gutter), currFloatToBeLocated.floatBounds[1]]);
                            }
                        else {
                    ...
                    //storing the float placed on page as an object
                    if (config.pageFloatsInfo[currPage.name] == null) {
                        config.pageFloatsInfo[currPage.name] = "BLK_"+ floatsToBeLocated[ftm];
                        }
                    else {
                        config.pageFloatsInfo[currPage.name] = config.pageFloatsInfo[currPage.name].concat("BLK_"+ floatsToBeLocated[ftm]);
                        }
                    }//end of FOR loop                
                //==========================================
                ...
                //reshift the placement of floats if the float width extends upto imaginary coulmns
                //====================================================================
                ResolveOversetInFloatPlacer(app.activeDocument);
                ...
                if (currPageName == 1){
                    ...
                    if (!(typeof(previousBreakFrame) === 'undefined')){
                        currPageFrame = previousBreakFrame;
                        }
                    else {
                        currPageFrame = currPage.textFrames.itemByName("RECTO");
                        }
...
//~                     currPageFrame = currPage.textFrames.itemByName("VERSO");
                ...
                if (currPageFrame.isValid){//checking whether the page has text frames for alignment
                    ...
                    if (!(currPageFrame.nextTextFrame == null)){
                        ...
                        //further checking if the currPageFrame has any table that is being continued to next page, 
                        //to do so need to check the last cell's page name if it is not equal to current page name then the table is continuing (first case)
                        //other situation would be current may have a coninued table and that would keep going, so for that a flag will set up to check condition (second case)
                        if (!nextFrameIsValid && currPageFrame.tables.length > 0){//this is first case
                            var lastTableOnFrame = currPageFrame.tables.lastItem();
                            var lastTableOnFrameLastCell = lastTableOnFrame.cells.lastItem();
                            var lastTableOnFrameLastCellPageName = lastTableOnFrameLastCell.insertionPoints[0].parentTextFrames[0].parentPage.name;
                            if(lastTableOnFrameLastCellPageName != currPage.name){
                                config.inlineTableContinuedUptoPage = lastTableOnFrameLastCellPageName;
                                config.inlineTableContinuedUptoPageID = lastTableOnFrame.id;
                                nextFrameIsValid = true;
                                }//end of IF
                            }//end of if 
                        else if (!nextFrameIsValid && (config.inlineTableContinuedUptoPage != currPage.name) && (config.inlineTableContinuedUptoPage != '')){//this is second case
                                nextFrameIsValid = true;
                            }//end of ELSE IF
                        }//end of IF
                    //=====================================
                    if (nextFrameIsValid == true && (typeof(previousBreakFrame) === 'undefined')){
                    ...
                    else if (nextFrameIsValid == false || !(typeof(previousBreakFrame) === 'undefined')){
                       ...
                       if(currPage.name == '1'){
                        ...
                        config["lastColumnLastParaSpanAndTable"] = false;//this has been added for dmfr0054 and dmfr0084 as temp solution on 10-Oct-2017, should be revisited
                        for (var lstb = 0; lstb < currPageFrameTextClmCt; lstb++){
                            if (currPageFrameTextClm[lstb].isValid){
                                ...
                                if (currPageFrameTextClm[lstb-1].isValid){//this has been added for dmfr0054 and dmfr0084 as temp solution on 10-Oct-2017, should be revisited
                                    if(currPageFrameTextClm[lstb-1].paragraphs.lastItem().tables.length > 0){//this has been added for dmfr0054 and dmfr0084 as temp solution on 10-Oct-2017, should be revisited
                                        config["lastColumnLastParaSpanAndTable"] = true;
                                        }
                                    }
                                }
                            }//end of FOR loop
                        ...
                        function resetPlacedFloatsBounds (shiftvalue,floatsBtmBounds){
                            ...
                            for (var ftm = 0; ftm < currPageBtmFloatsLen; ftm ++){
                                ...
                                if (currFloatToBeLocated.floatRotated){
                                    ...
                                    //
                                    if (currPage.name == 1 && currPageBtmFloatsLen){
                                        var currFrmTopWrap = config.currDoc.pageItems.itemByID(currFloatToBeLocated.frameID).textWrapPreferences.textWrapOffset[0];
                                        if (currFrmTopWrap > 0){
                                            shiftvalue = shiftvalue - (currFrmTopWrap);
                                            }
                                        }
                        ...
                        //==========================================
                       //===============================================================
                        if (!config.lastColumnLastParaSpanAndTable){//this has been added for dmfr0054 and dmfr0084 as temp solution on 10-Oct-2017, should be revisited
                            currPageFrame.textFramePreferences.verticalBalanceColumns = true;
                            }
                        var currPageFrameTextClmCt = currPageFrame.textFramePreferences.textColumnCount;
                        var currPageFrameTextClm = currPageFrame.textColumns;
                        var clmLastLineBaseArray = new Array ();
                        for (var lstb = currPageFrameTextClmCt - 1; lstb >= 0; lstb--){
                            if (currPageFrameTextClm[lstb].isValid){
                                if (currPageFrameTextClm[lstb].footnotes.length > 0){//code has been modified here on 16-Aug-2017 to consider the column footnote space for base align
                                    var clmBtmBound = currProcessClms[0].bounds[2];
                                    var docFootnoteSpacer = config.currDoc.footnoteOptions.spacer;
                                    var firstFNFLineBaseline = currPageFrameTextClm[lstb].footnotes[0].lines[0].baseline;
                                    var firstFNFLineAscent = currPageFrameTextClm[lstb].footnotes[0].lines[0].ascent;
                                    var currClmLastLineBase = currProcessClms[0].bounds[2];
                                    var totalFNSpace = currClmLastLineBase - (firstFNFLineBaseline - docFootnoteSpacer - firstFNFLineAscent);
                                    var currClmLastLineBase = currPageFrameTextClm[lstb].lines.lastItem().baseline + totalFNSpace;
                                    clmLastLineBaseArray = clmLastLineBaseArray.concat(currClmLastLineBase);
                                    }//end of IF
                                else if (config.currDocDOM.evaluateXPathExpression("//div[@class='jrnlFNTxtBlock']").length > 0 && currPage.name == 1){//this condition is specific for BMJ, has to rewritten later (modified on 17-Aug-2017)
                                    var clmBtmBound = currProcessClms[0].bounds[2];
                                    var docFootnoteSpacer = 12;//default value
                                    var firstFNFLineBaseline = config.currDocDOM.evaluateXPathExpression("//div[@class='jrnlFNTxtBlock']")[0].xmlItems[0].paragraphs[0].lines[0].baseline;
                                    var firstFNFLineAscent = config.currDocDOM.evaluateXPathExpression("//div[@class='jrnlFNTxtBlock']")[0].xmlItems[0].paragraphs[0].lines[0].ascent;
                                    var currClmLastLineBase = currProcessClms[0].bounds[2];
                                    var totalFNSpace = currClmLastLineBase - (firstFNFLineBaseline - docFootnoteSpacer - firstFNFLineAscent);
                                    var currClmLastLineBase = currPageFrameTextClm[lstb].lines.lastItem().baseline + totalFNSpace;
                                    clmLastLineBaseArray = clmLastLineBaseArray.concat(currClmLastLineBase);
                        ...
                        floatPlacerLastProcessedFrame = currPageFrame;
                        floatPlacerLastProcessedFrameBounds = currPageFrame.geometricBounds;
                        currPageFrame.textFramePreferences.verticalBalanceColumns = false;
                        if(config.currDoc.pages[parseInt(currPage.name)].isValid && !nextFrameIsValid){
                            config.currDoc.pages[parseInt(currPage.name)].remove();
                        }
                        config["lastPageTextFrameAdjusted"] = true;
                        if (currPage.name == 2 && config.currDoc.pages[2].isValid){
                            config.currDoc.pages[2].remove();
                            }
                        else if (currPage.name == 1){
                            if (config.currDoc.pages[2].isValid){
                                config.currDoc.pages[2].remove();
                                }
                            if (config.currDoc.pages[1].isValid){
                    ...
                    config["noSpanColumns"] = true;
                    if (currPageFrame.isValid){
                        //before cheking for any span property for the frames, need to check whether the text frame has inline tables and if so need to check to how many column the table would span, then the span property of it's paragraph should be modified to equivalent that
                        var currPageFrameParas = currPageFrame.paragraphs;
                        var currPageFrameParasLen = currPageFrameParas.length;
                        for (var fps = currPageFrameParasLen - 1; fps >= 0; fps --){
                            var currPageFrameCurrPara = currPageFrameParas[fps];
                            if(currPageFrameCurrPara.tables.length > 0){
                                if(currPage.name == '1'){
                                    var currProcessClmsWidth = config.openerPageColumnsSpanWidth;
                                    var currProcessClmsWidthLen = config.openerPageColumnsLen;
                                    }//end of IF
                                ...
                                var currPageFrameCurrParaTbl = currPageFrameCurrPara.tables[0];
                                var currPageFrameCurrParaTblWidth = currPageFrameCurrParaTbl.width;
                                for (var tblwd = 0; tblwd < currProcessClmsWidthLen; tblwd ++){
                                    if ((currPageFrameCurrParaTblWidth <= currProcessClmsWidth[tblwd][0].width) || (tblwd == (currProcessClmsWidthLen - 1))){
                                        currPageFrameCurrPara.spanColumnType = SpanColumnTypeOptions.SPAN_COLUMNS;
                                        currPageFrameCurrPara.spanSplitColumnCount = tblwd + 1;
                                        ResolveOverset(app.activeDocument,retriggeringFloatPlacer,currPageFrame);
                                        if (config.inlineTableContinuedUptoPageID == currPageFrameCurrParaTbl.id){
                                            config.inlineTableContinuedUptoPage = currPageFrameCurrParaTbl.cells.lastItem().insertionPoints[0].parentTextFrames[0].parentPage.name;
                                            }
                                        break;
                                        }//end of IF
                                    }//end of FOR loop
                        ...
                        //checking whether currFrmActColCnt and currFrmSpanColLen are equal
                        if (currFrmActColCnt == currFrmSpanColLen){
                            config.noSpanColumns = false;
                            }//end of IF
...
//~                                 }//end of ELSE IF
                    ...
                    if (!(config.baseAlignmentDetails.clmShortBy == 0) || ((config.baseAlignmentDetails.clmShortBy == 0) && (config.baseAlignmentDetails.numOfLinesOnCurrClm < config.minNoLinesOnPag))){
                    ...
                    floatPlacerLastProcessedFrame = config.currDoc.textFrames.itemByID(currPageFrameID);
                    floatPlacerLastProcessedFrameBounds = config.currDoc.textFrames.itemByID(currPageFrameID).geometricBounds;
    ...
    baseAlignment: function(baseAlignmentDetails, currPageName, clmsForAlign, maxBaselinePositionArray, currPageFrameID, currClm){
        ...
        if (numOfLines < config.minNoLinesOnPag && floatFrmsOnClmLen > 0){//Case 1
            ...
            if(floatFrmsOnClmLen > 0 && (config.lastPageTextFrameAdjusted == false)){//if no of floats on the clm is greater than 0, proceeding further
                ...
                if (config.proceedFloatsAdjust){//if proceedFloatsAdjust is true then there might be some balance space available for headlevel adjustment or for the further space adjusment
                    adjustingHeadlevels(config.shortByValueBalance, lastLineLeading, adjustSpacingFor);
                    if ((config.proceedHeadSpaceAdjust || config.shortByValueBalance != 0) && (config.shortByValueBalance > lastLineLeading)){//still there are some space to be distributed
                        if (config.shortByValueBalance == 2*lastLineLeading){//checking whether the still the shortByValue is equal twice the last line leading 
                            trackParaCt = trackParaCt;
                            }//end of IF
                        else {
                    ...
                    else if ((config.shortByValueBalance != 0) && (config.shortByValueBalance < lastLineLeading)){
                        VJadjustment(config.shortByValueBalance);
                            if (config.shortByValueBalance == 0){
                                config.shortByValueBalance = 0;
                                config.proceedFloatsAdjust = true;
        ...
        //config.pageColumnBaseAlignInfo[currPage.name] = {currClm:config.shortByValueBalance};
...
//~                 tempFrame.remove();
                ...
                if (!currLibItem[0].toString().match("Object TextFrame","i")){
                    for (var mov = 0; mov < movingObjLen; mov ++){
                ...
                else if (currLibItem[0].toString().match("Object TextFrame","i")){
                        ...
                        myDoc.recompose();
                        movingObj = [];
                        movingObj = movingObj.concat(currLibItem[0]);
                        movingObjLen = movingObj.length;
                    }
                ...
                for (var cl = 0; cl < movingObjLen; cl ++){
                    ...
                    var frmFirstParaPointSize = frmFirstPara.pointSize;
                    var frmFirstParaLeading = frmFirstPara.leading;
                    var frmFirstParaTracking = frmFirstPara.tracking;
                    if(!(frmFirstParaStyle == undefined)){
                        frmFirstPara.appliedParagraphStyle = config.currDoc.paragraphStyles.itemByName(frmFirstParaStyle);
                        frmFirstPara.tracking = frmFirstParaTracking;
                        frmFirstPara.pointSize = frmFirstParaPointSize;
                        frmFirstPara.leading = frmFirstParaLeading;
                        }//end of IF
                    var frmLastPara = currFrm.paragraphs.lastItem();
                    var frmLastParaStyle = frmLastPara.appliedParagraphStyle.name;
                    var frmLastParaTracking = frmLastPara.tracking;
                    var frmLastParaPointSize = frmLastPara.pointSize;
                    var frmLastParaLeading = frmLastPara.leading;
                    if (frmParaLen > 1){
                    ...
                    if (frmLastPara.characters.lastItem().contents == '\r'){
                        frmLastPara.characters.lastItem().contents = "";
                        frmLastPara.appliedParagraphStyle = frmLastParaStyle;
                        frmLastPara.tracking = frmLastParaTracking;
                        frmLastPara.pointSize = frmLastParaPointSize;
                        frmLastPara.leading = frmLastParaLeading;
                        }//end of IF
                    else if (frmParaLen > 1){
                        if (frmPenultPara.characters.lastItem().contents == '\r'){
                            var frmPenultParaTracking = frmPenultPara.tracking;
                            var frmPenultParaPointSize = frmPenultPara.pointSize;
                            var frmPenultParaLeading = frmPenultPara.leading;
                            frmPenultPara.characters.lastItem().contents = "";
                            frmPenultPara.appliedParagraphStyle = frmPenultParaStyle;
                            frmPenultPara.tracking = frmPenultParaTracking;
                            frmPenultPara.pointSize = frmPenultParaPointSize;
                            frmPenultPara.leading = frmPenultParaLeading;
                            }
                        }//end of ELSE IF
                ...
                config.currDoc.viewPreferences.rulerOrigin = RulerOrigin.pageOrigin;
                //=============================================
                if (TCHContFrm !=undefined){
                    TCHContFrmBoundsAfter = TCHContFrm.geometricBounds;
                    TCContFrmBoundsAfter = TCContFrm.geometricBounds;
                    }
                else{
                    TCHContFrmBoundsAfter = [0,0,0,0];//this has to corrected later
                    TCHContFrmBoundsBefore = [0,0,0,0];//this has to corrected later
                    TCContFrmBoundsAfter = TCContFrm.geometricBounds;
                    }
                //adjusting 'TEXT_CONTAINER' bounds if required
                if (TCHContFrmBoundsAfter[2] > TCHContFrmBoundsBefore[2] && (TCHContFrm !=undefined)){
                ...
                else if (TCHContFrmBoundsAfter[2] < TCHContFrmBoundsBefore[2] && (TCHContFrm !=undefined)){
...
function VJadjustment(clmShortBy){
    ...
    var toleranceValue = (clmShortBy)/100
    var valuesToBeDistributed = clmShortBy - toleranceValue;
    var actualValuesToBeDistributed = valuesToBeDistributed/totalLinesLen;
    ...
    if(parseFloat(actualValuesToBeDistributed) <= parseFloat(maxPercent)){
...
function increaseParaSpace(shortBy, adjustSpacingFor){
    ...
    for (var p = 0; p < parasOnTheClmLen; p ++){
        ...
        if (!(spaceAllowed == 0) && (p > 0 || (currPage.name == 1 && (config.noSpanColumns)))){
...
//~                                 baseLineArray.push(lastRowLineBase);
                            ...
                            if(!(tableFrameCurrInsetSpMinHt >= lastRowBottomInsetAddBase)){                                
                                ...
                                allRows[r-2].cells.everyItem().fillColor = config.currDoc.swatches.itemByName(currLastRowFillColor);//dt 05-Jul-2017, earlier code has direct color name now changed to pick the color name from document's color list item
                                //then checking whether the current row has any span cell if so then have to apply color separately, for now just checking whether the immediate previous  has span rows (applying same cell fill color for that); updated on 21-Jul-2017
                                //================================================================================
                                var currRowCellCt = allRows[r-2].cells.length;
                                if (currRowCellCt < currTableClmLen){
                                    var preRowCells = allRows[r-3].cells;
                                    ...
                                    for (var pr = 0; pr < preRowCellsLen; pr ++){
                                        var currPrevRowCell = preRowCells[pr];
                                        if (currPrevRowCell.rowSpan > 1){
                                            currPrevRowCell.fillColor = config.currDoc.swatches.itemByName(currLastRowFillColor);
                                            }//end of IF
                                        }//end of FOR loop
                                    }//end of IF                                
                                //================================================================================
...
//~                                 nextFirstRowAppliedStyle = allRows[r-1].cells.everyItem().appliedCellStyle = "TBF"; //Applied table cell style in the last row here
                               ...
                               tableFrameNameCurr.fit(FitOptions.FRAME_TO_CONTENT)
                                ...
                                lastRowAppliedStyle = allRows[r-1].cells.everyItem().appliedCellStyle = tableLastRowStyle; //Applied table cell style in the last row here
                                allRows[r-1].cells.everyItem().fillColor = config.currDoc.swatches.itemByName(currLastRowFillColor);//dt 05-Jul-2017, earlier code has direct color name now changed to pick the color name from document's color list item
                                //then checking whether the current row has any span cell if so then have to apply color separately, for now just checking whether the immediate previous  has span rows (applying same cell fill color for that); updated on 21-Jul-2017
                                //================================================================================
                                var currRowCellCt = allRows[r-1].cells.length;
                                if (currRowCellCt < currTableClmLen){
...
//******************* Start Group *********************   
    if(firstItteration){
        ...
        else if(config.carryForwardFromPage2){
            config.pageFloatIDsArray = config.pageFloatIDsArray.concat([currPageCitationCont]);            
            }
        else {
    ...
    if(repeatheader == true){
        ...
        var results = tableFrameNameCurr.findText(); //Find content based on the paragraphs syle (Caption style)
            ...
            for(var i=0; i<results.length; i++) {
                ...
                continueCapAdd.move([tableFrameNextBoundValueX1, tableFrameNextBoundValueY1-tableFrameNextBoundValueY2])
                tableFrameNameCurrCap.duplicate(LocationOptions.AT_END, continueCapAdd.insertionPoints[0])
                continueCapAdd.paragraphs[0].characters.lastItem().insertionPoints[-1].contents = symbol;
                try{
                    continueCapAdd.appliedObjectStyle = curDoc.objectStyles.itemByName(floatCapObjStyle);
                }catch(e){};
                var contStyle = continueCapAdd.paragraphs[0].characters.lastItem().insertionPoints[-1].contents = headContText;
...
//==============================================
//figure block continued function
function figureContinuedBlock(figureFrameNameCurr,currPageName,boundsForContFrms,currCitation, hzc, floatSpnTo, currCitBaseline, rotateFloat, wdSplitEqually, defaultFloatContCt, currCitBlkHt, figObjStyle,config,currProcessClms){
    var curDoc = app.activeDocument;
    wrapTop = config.wrapAroundFloat.top;
    wrapLeft = config.wrapAroundFloat.left;
    wrapBottom = config.wrapAroundFloat.bottom;
    wrapRight = config.wrapAroundFloat.right;
    resetBounds = figureFrameNameCurr.geometricBounds;
    var figureFrameNameCurrY1 = boundsForContFrms[0];
    var figureFrameNameCurrX1 = boundsForContFrms[1];
    var figureFrameNameCurrY2 = boundsForContFrms[2];
    var figureFrameNameCurrX2 = boundsForContFrms[3];
    var pagesWidth = config.pageSize['width']; //Template Page Width
    var pagesHeight = config.pageSize['height']; //Template Page height
    var marginTop = config.margin['top']; //y1
    var marginBottom = config.margin['bottom']; //y2
    var marginInside = config.margin['inside']; //x2
    var marginOutside = config.margin['outside']; //x1
    //may need further changes here when goes for multi pages
        figureFrameCurr = figureFrameNameCurr; 
        figureFrameNext = curDoc.pages.itemByName(currPageName).textFrames.add({geometricBounds: [figureFrameNameCurrY1, figureFrameNameCurrX1, figureFrameNameCurrY2, figureFrameNameCurrX2]}); //Creating Next Text Frame here
        figureFrameNext.move([figureFrameNameCurrX1, figureFrameNameCurrY1])
        figureFrameNext.previousTextFrame = figureFrameCurr; //In link for current frame to Next frame.
        //updating any missing glyphs==================================
        config.currDoc.viewPreferences.rulerOrigin = RulerOrigin.spreadOrigin;
        var tempBound = figureFrameNext.geometricBounds;
        figureFrameNext.textWrapPreferences.textWrapMode = TextWrapModes.NONE;
        figureFrameNext.move(currPage);
        currTextFrameObjectForGlyphSearch = figureFrameNext;
        app.doScript(File(layerTemplateScript + "\\"+ "missingGlyph.jsx"), ScriptLanguage.javascript, [layerTemplateScript, currTextFrameObjectForGlyphSearch]);
        figureFrameNext.move([tempBound[1], tempBound[0]]);
        config.currDoc.viewPreferences.rulerOrigin = RulerOrigin.pageOrigin;
        //===================================================================================
        figureFrameCurr.textFramePreferences.autoSizingType = AutoSizingTypeEnum.HEIGHT_ONLY; //Fix Text Frame Height here.
        figureFrameCurr.textFramePreferences.autoSizingType = AutoSizingTypeEnum.OFF;
        figureFrameCurr.fit(FitOptions.FRAME_TO_CONTENT);
        //===================================================================================
        //now getting the last paragraphs last line base to fit the frame bounds (btm bound)
        figureFrameNext.appliedObjectStyle = curDoc.objectStyles.itemByName(figObjStyle);
        var figureFrameNextLastLineBase = figureFrameNext.lines.lastItem().baseline;
        var figureFrameNextCurrBnd = figureFrameNext.geometricBounds;
        figureFrameNext.geometricBounds = [figureFrameNextCurrBnd[0],figureFrameNextCurrBnd[1],figureFrameNextLastLineBase,figureFrameNextCurrBnd[3]];
        //===================================================================================
        var figureFrameCurrBounds = figureFrameCurr.geometricBounds;
        //adding continued caption
        app.findTextPreferences = NothingEnum.nothing; 
        app.changeTextPreferences = NothingEnum.nothing;
        app.findGrepPreferences.findWhat = ".+";
        app.findGrepPreferences.appliedParagraphStyle = 'jrnlFigCaption';
        var searchCaption = figureFrameCurr.parentStory.findGrep();
        var figureLabelText = searchCaption[0].paragraphs[0].contents.replace(/(Figure\s\d+).+/,'$1');
        captionContinuedFrame = curDoc.pages.itemByName(currPageName).textFrames.add({geometricBounds: [figureFrameNameCurrY1, figureFrameNameCurrX1, figureFrameNameCurrY2, figureFrameNameCurrX2]}); //Creating Next Text Frame here
        captionContinuedFrame.geometricBounds = [figureFrameCurrBounds[2] + 5,figureFrameCurrBounds[1], figureFrameCurrBounds[2]+ 20 + 5, figureFrameCurrBounds[3]];//here need mofication for the values added to placed continued frame space
        captionContinuedFrame.textFramePreferences.ignoreWrap = true;
        captionContinuedFrame.insertionPoints[0].contents = figureLabelText;
        captionContinuedFrame.characters.lastItem().insertionPoints[-1].contents =  SpecialCharacters.EM_SPACE;
        captionContinuedFrame.characters.lastItem().insertionPoints[-1].contents =  'Continued';
        captionContinuedFrame.paragraphs[0].appliedParagraphStyle = 'jrnlFigCaption';
        captionContinuedFrame.textFramePreferences.autoSizingType = AutoSizingTypeEnum.HEIGHT_ONLY; //Fix Text Frame Height here.          
        curDoc.recompose();
        //===================================================================================
        //grouping the figure frame and caption continued frame
        var groupArray = new Array();
        groupArray.push(figureFrameCurr);
        groupArray.push(captionContinuedFrame);
        var groupBlock = config.currDoc.groups.add(groupArray);
        //===================================================================================
        tempFigureFrmID = groupBlock.id;
        var groupBlockBounds = groupBlock.geometricBounds;
        floatBlockWd = groupBlockBounds[3] - groupBlockBounds[1];
        floatBlockHt = groupBlockBounds[2] - groupBlockBounds[0];
        floatBlockArea = floatBlockWd * floatBlockHt;
        //when the float is being continued then entire text on that particular page/column will be pushed out by restting the bottom wrap to the bottom of page/column
        var pageClmnHt = currProcessClms[0].height;
        var revisedWrapBottom = pageClmnHt - floatBlockHt;
        groupBlock.textWrapPreferences.textWrapMode = TextWrapModes.NONE;
        groupBlock.textWrapPreferences.textWrapMode = TextWrapModes.BOUNDING_BOX_TEXT_WRAP;
        groupBlock.textWrapPreferences.textWrapOffset = [wrapTop, wrapLeft, revisedWrapBottom, wrapRight];        
        config.rotateFloat = rotateFloat;
        //===================================================================================
        figureFrameNext.textWrapPreferences.textWrapMode = TextWrapModes.NONE;
        figureFrameNext.textWrapPreferences.textWrapMode = TextWrapModes.BOUNDING_BOX_TEXT_WRAP;
        figureFrameNext.textWrapPreferences.textWrapOffset = [wrapTop, wrapLeft, wrapBottom, wrapRight];        
        config.floatContinued = false;
        config.continuedFloatDetails = {};
        var figureFrameCurrBounds = figureFrameNext.geometricBounds;
        var figureFrameBlockHt =  figureFrameCurrBounds[2] - figureFrameCurrBounds[0];
        var figureFrameBlockWd = figureFrameCurrBounds[3] - figureFrameCurrBounds[1];
        var figureFrameBlockArea = figureFrameBlockHt * figureFrameBlockWd;
        if (config.figureContinuedBlockItteration == 0){//just storing very first frame's info
            config.firstFigureFrameInfo = {"firstFloatBlockWidth":floatBlockWd,"firstFloatBlockHeight":floatBlockHt,"firstFloatBlockArea":floatBlockArea,"firstTempFigureFrmID":tempFigureFrmID}
            }
        //adding caption continued to the footer
        wrapBottom = revisedWrapBottom;
        if (figureFrameNext.overflows){
            figureFrameNameCurr = figureFrameNext;
            if(config.figureContinuedBlockItteration > 0){
                config.continuedBoxID.push(currCitation + '_' + config.figureContinuedBlockItteration);
                config.continuedBoxDetails[currCitation + '_' + config.figureContinuedBlockItteration] = {"citationBlockHeight":0,"floatBlockHeight":floatBlockHt, "floatBlockWidth":floatBlockWd, "floatBlockArea":floatBlockArea,"floatSpanTo":floatSpnTo,"frameID":tempFigureFrmID, "floatPlaceFrom":'', "onPage":currPage.name, "currCitBaseline":0, 'floatRotated':rotateFloat, "landscapeAdditionalOffset":0, "wrapTop":wrapTop, "wrapBottom":wrapBottom, "placingSequence":config.placingSequence, "preferredOnColumn":config.preferredOnColumn};
                }
            config.figureContinuedBlockItteration ++;
            figureContinuedBlock(figureFrameNameCurr,currPageName,boundsForContFrms,currCitation, hzc, floatSpnTo, currCitBaseline, rotateFloat, wdSplitEqually, defaultFloatContCt, currCitBlkHt, figObjStyle,config,currProcessClms);
            }
        else {
            if(config.figureContinuedBlockItteration == 1){
                config.continuedBoxID.push(currCitation + '_' + 0);
                config.continuedBoxDetails[currCitation + '_' + 0] = {"citationBlockHeight":0,"floatBlockHeight":floatBlockHt, "floatBlockWidth":floatBlockWd, "floatBlockArea":floatBlockArea,"floatSpanTo":floatSpnTo,"frameID":tempFigureFrmID, "floatPlaceFrom":'', "onPage":currPage.name, "currCitBaseline":0, 'floatRotated':rotateFloat, "landscapeAdditionalOffset":0, "wrapTop":wrapTop, "wrapBottom":wrapBottom, "placingSequence":config.placingSequence, "preferredOnColumn":config.preferredOnColumn};
                }
            config.continuedBox = true;
            config.continuedBoxID.push(currCitation + '_' + config.figureContinuedBlockItteration);
            config.continuedBoxDetails[currCitation + '_' + config.figureContinuedBlockItteration] = {"citationBlockHeight":0,"floatBlockHeight":figureFrameBlockHt, "floatBlockWidth":figureFrameBlockWd, "floatBlockArea":figureFrameBlockArea,"floatSpanTo":floatSpnTo,"frameID":figureFrameNext.id, "floatPlaceFrom":'', "onPage":currPage.name, "currCitBaseline":0, 'floatRotated':rotateFloat, "landscapeAdditionalOffset":0, "wrapTop":wrapTop, "wrapBottom":wrapBottom, "placingSequence":config.placingSequence, "preferredOnColumn":config.preferredOnColumn};
            floatBlockWd = config.firstFigureFrameInfo.firstFloatBlockWidth;
            floatBlockHt =  config.firstFigureFrameInfo.firstFloatBlockHeight;
            floatBlockArea =  config.firstFigureFrameInfo.firstFloatBlockArea;
            tempFigureFrmID = config.firstFigureFrameInfo.firstTempFigureFrmID;
            }
        return floatBlockWd, floatBlockHt, floatBlockArea,tempFigureFrmID,config.rotateFloat, wdSplitEqually, config.floatContinued, config.continuedFloatDetails, wrapTop, wrapBottom;
    }
//==============================================
//Function for removing overset - to be deleted later
function ResolveOversetInFloatPlacer(currDoc)
{
...
//~      app.activeWindow.zoomPercentage = 100;
     ...
     if (retriggeringFloatPlacer){//this condition has been added on 18-Aug-2017 to continue run resolveoverset while placing floats for run on articles
         firstFrame = floatPlacerLastProcessedFrame;
         }//end of IF
...
function imageSizer(frmObj, imageData) {
    ...
    frmObj.textFramePreferences.ignoreWrap = true
    ...
    for (currParaIndex; currParaIndex < paraCount; currParaIndex++){
        ...
        if (/^[\uFFFC\uFEFF]+[\r\n]*$/.test(currPara.contents) && !imageOnTop) {
            ...
            var currParaStyleLeading = currParaStyle.leading;
            if (currParaStyle.leading == 1635019116){
                currParaStyleLeading = currParaStyle.pointSize * 1.2;
                }
            var spaceUsedByText = spaceUsedByText + currParaStyle.spaceBefore + currParaStyle.spaceAfter + (currParaStyleLeading * linesCount);            
        }
    }
...
function splitBackmatterTextframe(xPathsInfo, changeParaSpanAttrib, currPage, clmGutter){
    ...
    if(nodeToBeMoved){//if the xpath is valid
        ...
        previousBreakFrame = mainFrm;
        return previousBreakFrame;
        }
}
// *********************************************************************************************************
/*Developer: Augustine. K
...
//===========================================================================================================
/*
applyinhg span rule for table col head
Developer: Kannan R
Code deployed on: 14-Jul-2017
*/
function _table_colspan_stroke(table_frame, stroke_box_width)
{
            doc = app.activeDocument;
            if((table_frame) && (table_frame.toString().match("Object TextFrame","i")) && (table_frame.tables[0].isValid))
            {
                   var t = 0;
                   for(t=0;t<table_frame.tables.length;t++)
                   {
                                    var tbl = table_frame.tables[t];
                                    //var col = tbl.columns.length;
                                    var col_span = tbl.cells.everyItem().columnSpan;
                                    for(var c in col_span)
                                    {
                                            var cell = tbl.cells[c];
                                            var last_cell = cell.rows[0].cells.lastItem();
                                            var next_row = tbl.rows.nextItem(tbl.cells[c].parentRow)
                                           
                                           // columnspan should be morethan 1,
                                           //cell's pstyle should be "jrnlTblHead" as well as next row pstyle also,
                                           //And  cell should not last column
                                           //An additional condition has been added to check the cell doesn't have rowSpan (dt: 26-Jul-2017)
                                            if( (col_span[c] > 1) && (cell.insertionPoints[0].appliedParagraphStyle.name.toString().match("jrnlTblHead")) && (next_row.cells[0].insertionPoints[0].appliedParagraphStyle.name.toString().match("jrnlTblHead")) && (cell != last_cell) && (cell.rowSpan == 1))
                                            {
                                                    var cell_bottom_stroke_weight = cell.bottomEdgeStrokeWeight;
                                                    if( (cell_bottom_stroke_weight) && (cell_bottom_stroke_weight > 0) ) // cell's bottom stroke > 0
                                                    {
                                                                var cell_color = cell.fillColor;
                                                                var cell_bottom_inset = cell.bottomInset;
                                                                var IP = cell.insertionPoints[0];
                                                                var stroke_box_height = (cell_bottom_stroke_weight + 1);
                                                                doc.select(null);
                                                                
                                                                if(stroke_box_height < 3)
                                                                    stroke_box_height = 3;
                                                                if(stroke_box_width < 3)
                                                                    stroke_box_width = 3;
                                                                
                                                                var tf = doc.textFrames.add({geometricBounds:[0,0,stroke_box_height,stroke_box_width],fillColor:cell_color})
                                                                // Anchored object settings
                                                                with(tf.anchoredObjectSettings)
                                                                {
                                                                        insertAnchoredObject(IP,AnchorPosition.ANCHORED);
                                                                        ...
                                                                        anchorXoffset = 0;
                                                                        anchorYoffset = (cell_bottom_inset + (stroke_box_height/2)) + " pt";
...
//===========================================================================================================
//function for fixing side caption
function _side_caption(page_item_id, frame_height,return_value,currWrapInfo)
{
            doc = app.activeDocument;
            try
            {
                    page_item_frame = "";
                    page_item = doc.pageItems.itemByID(parseInt(page_item_id));
                    doc.select(page_item);
                    page_item_frame = doc.selection[0];
                    //$.writeln(page_item.name)
            }
            catch(e){}
            if( (page_item_frame) && (page_item_frame.toString().match("textFrame","i")) )
            {
                    // Get values from config file.
                        graphic_max_percent = config.sideCaption.maximumPercent;
                        gutter_value = config.sideCaption.gutter;
                        side_caption_align = config.sideCaption.alignTo;
                       
                        if( (graphic_max_percent) && (gutter_value))
                        {
                                            var page_item_bounds = page_item.geometricBounds;
                                            var frame_width = page_item.geometricBounds[3] - page_item.geometricBounds[1];
                                            var frame_height = page_item.geometricBounds[2] - page_item.geometricBounds[0];
                                            var graphic = page_item.allGraphics[0];
                                            if(graphic)
                                            {
                                                       var graphic_frame = graphic.parent;
                                                       var graphic_frame_width = graphic_frame.geometricBounds[3] - graphic_frame.geometricBounds[1];
                                                       var graphic_frame_height = graphic_frame.geometricBounds[2] - graphic_frame.geometricBounds[0];
                                                       var graphic_checking_value = (frame_width * (graphic_max_percent / 100));
                                                      // $.writeln( graphic_frame_width+ " = " + graphic_checking_value)
                                                       if(graphic_frame_width <= graphic_checking_value)
                                                       {
                                                                  page_item.geometricBounds = [page_item.geometricBounds[0],page_item.geometricBounds[1],(graphic_frame_height +page_item.geometricBounds[0]),(graphic_frame_width+page_item.geometricBounds[1])]
                                                                  var side_caption_frame = page_item.duplicate(); 
                                                                  side_caption_frame.associatedXMLElement.remove();
                                                                  side_caption_frame_width = frame_width - gutter_value - graphic_frame_width;
                                                                  side_caption_frame.textFramePreferences.textColumnFixedWidth = side_caption_frame_width;
                                                                  //$.writeln(side_caption_frame_width)
                                                                  side_caption_frame.move([side_caption_frame.geometricBounds[1] - side_caption_frame_width - gutter_value,side_caption_frame.geometricBounds[0],]);
                                                                  page_item.nextTextFrame = side_caption_frame;
                                                                  if(side_caption_frame.overflows)
                                                                  {
                                                                                side_caption_frame.remove();
                                                                                page_item.geometricBounds = page_item_bounds;
                                                                                return_value = [page_item_id, frame_height];
                                                                                return return_value;//"Caption frame is overflows";
                                                                  }
                                                                  else
                                                                  {
                                                                              if(side_caption_align)
                                                                              {
                                                                                        if(side_caption_align.toString().match("top","i"))
                                                                                                side_caption_frame.textFramePreferences.verticalJustification = VerticalJustification.TOP_ALIGN;
                                                                                        else if(side_caption_align.toString().match("bottom","i"))
                                                                                                side_caption_frame.textFramePreferences.verticalJustification = VerticalJustification.BOTTOM_ALIGN;
                                                                              }
                                                                              var group_page_item_arr = [page_item,side_caption_frame];
                                                                              var group_frame = doc.groups.add(group_page_item_arr);
                                                                              var group_frame_height = group_frame.geometricBounds[2] - group_frame.geometricBounds[0];
                                                                              doc.select(null);
                                                                              //$.writeln(group_frame.id);
                                                                              group_frame.textWrapPreferences.textWrapMode = TextWrapModes.BOUNDING_BOX_TEXT_WRAP;
                                                                              group_frame.textWrapPreferences.textWrapOffset = [currWrapInfo[0], currWrapInfo[1], currWrapInfo[2], currWrapInfo[3]];
                                                                              return_value = [group_frame.id,group_frame_height]
                                                                              return return_value;
                                                                  }
                                                       }
                                                       else
                                                       {
                                                                   return_value = [page_item_id, frame_height];
                                                                   return return_value;//"Graphic is not fit for side caption";
                                                       }
                                            }
                                            else
                                            {
                                                        return_value = [page_item_id, frame_height];
                                                        return return_value;//"Graphic is not available";
                                            }
                        }
                        else
                        {
                                return_value = [page_item_id, frame_height];
                                return return_value;//"Some values are missing in config file (sideCaption:gutter|maximumPercent)"
                        }
            }
            else
            {
                        return_value = [page_item_id, frame_height];
                        return return_value;//"PageItem is not valid";
            }
