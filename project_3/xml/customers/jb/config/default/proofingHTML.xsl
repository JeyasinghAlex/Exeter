<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:mml="http://www.w3.org/1998/Math/MathML" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xlink="http://www.w3.org/1999/xlink" version="2.0" exclude-result-prefixes="xsi xs xlink mml">
   ...
   <!-- <xsl:template match="*[contains(@*, Punc)]|*[contains(@*, removeString)]"> -->
    ...
    <xsl:template match="//div[@class='body']//*[@data-content-type='label'][./parent::*[contains(@class,'jrnlHead')]]" priority="6">
...
<!--
    ...
    <!--<xsl:template match="*[@class='jrnlTblBlock']" priority="5">
        <xsl:apply-templates select="./node()"/>
    </xsl:template>-->
    <xsl:template match="//*[@class='body']/*[@class='jrnlBoxBlock']|//*[@class='back']/*[@class='jrnlAppBlock']/*[@class='jrnlBoxBlock']" priority="5">
        <div>
            <xsl:choose>
  ...
  <!--  <xsl:template match="div[@class='body'][./*[@class='jrnlBoxBlock'][not(.//*[@class='label'])]]/*[not(matches(@class, 'jrnl(.*?)Block'))][following-sibling::*[@class='jrnlBoxBlock'][not(.//*[@class='label'])]][last()]" priority="6">
    ...
    <xsl:template match="//div[@class='floatBlock']/*[@class='jrnlFootNoteFN']" priority="6">
        <div class="jrnlFootNoteFN">
            <xsl:apply-templates select="./@*"/>
            <p class="jrnlFootNotePara">
                <xsl:apply-templates select="./*[@class='jrnlFootNotePara']/@*"/>
                <span class="jrnlFootNoteHead">
                    <xsl:apply-templates select="./*[@class='jrnlFootNoteHead']/@*|./*[@class='jrnlFootNoteHead']/text()"/>
                </span>
                <xsl:apply-templates select="./*[@class='jrnlFootNotePara']/node()"/>
            </p>
        </div>
    </xsl:template>
    ...
    <xsl:template match="*[@class='body']//*[contains(@class,'jrnlHead')]" priority="5">
        <xsl:element name="{name(.)}">
            <xsl:apply-templates select=" @* "/>
            ...
            <xsl:when test="./preceding-sibling::label[1]">
                    ...
                    <xsl:variable name="fs" select="generate-id(./following-sibling::*[contains(@class,'jrnlHead')][1])"/>
                     
                    <xsl:if test="$currid = $fs">
                        <span class="label">
                            <xsl:apply-templates select="./node()"/>
                            <xsl:text>&#x2003;</xsl:text>
                        </span>
    ...
    <xsl:template match="*[@class='body']/label[following-sibling::*[contains(@class,'jrnlHead')][1]]" priority="8"/>
    <xsl:template match="*[@class='floatBlock']/*[@class='jrnlFootNote']/*[@class='jrnlFNHead']" priority="5">
        <xsl:choose>
            <xsl:when test="$journal ='jb'"/>
            <xsl:otherwise>
                <xsl:element name="{name(.)}">
                    <xsl:apply-templates select="@*|node()"/>
                </xsl:element> 
        ...
        <xsl:template match="*[@class='floatBlock']/*[@class='jrnlFootNote']/*[@class='jrnlFootNotePara']" priority="5">
        <xsl:choose>
            <xsl:when test="$journal ='jb'">
                <p>
                    <xsl:apply-templates select="@* | node()"/>
    ...
    <xsl:template match="*[@class='body']/*[@class='jrnlBlockQuote']" priority="5">
    <xsl:choose>
        <xsl:when test="$journal ='jb'">
            <xsl:element name="{name(.)}">
            <xsl:apply-templates select=" @*|node() "/>
                <xsl:if test="following-sibling::*[@class='jrnlDispAttrib'][1]">
                    <xsl:for-each select="following-sibling::*[@class='jrnlDispAttrib'][1]">
                        <xsl:value-of select="'&#x9;'"/>
                        <span class="jrnlDispAttrib">
                            <xsl:apply-templates select="./node()"/>
                        </span>
                    </xsl:for-each>
                </xsl:if>
    ...
    <xsl:template match="*[@class='body']//*[@class='jrnlDispAttrib']" priority="8">
        <xsl:choose>
            <xsl:when test="$journal ='jb'"/>
            <xsl:otherwise>
                <xsl:element name="{name(.)}">
    ...
    <xsl:template match="//div[@class='body1']//p[@data-class='example-stmt' or @data-class='example-stmt-sub' or @data-class='example-stmt-sub-sub']" priority="6"> 
        <xsl:element name="{name(.)}">
            <xsl:apply-templates select=" @*|./node()except(*[@class='jrnlStyledContent'])"/>
            <xsl:variable name="jrnlStyledContent" select="./*[@class='jrnlStyledContent']"/>
                <xsl:if test="./child::*[@class='jrnlStyledContent']">
                    <span>
                        <xsl:value-of select="'&#x9;'"/>
                    </span>
                </xsl:if>
            <xsl:apply-templates select="$jrnlStyledContent"/>
        </xsl:element>
    </xsl:template>
    <xsl:template match="//*[@class='jrnlTblBlock'][@data-type='Example']//table//tr" priority="6">
        <xsl:element name="{name(.)}">
        <xsl:apply-templates select="@*"/>
        <xsl:choose>
        <xsl:when test="preceding-sibling::tr">
            <xsl:for-each select=".//td">
                    <td>
                        ...
                        <xsl:for-each select=".//p">
                            <p>
                                <xsl:attribute name="class">
                                    <xsl:value-of select="'jrnlExmTblBody'"/>
                                </xsl:attribute>
                                <xsl:apply-templates select=" ./node() "/>
                            </p>
                        </xsl:for-each>
        ...
        <xsl:otherwise>
            <xsl:for-each select=".//td">
                <td>
                    <xsl:apply-templates select="@*"/>
                    <xsl:choose>
                        <xsl:when test="preceding-sibling::td or ./parent::tr/parent::tbody[preceding-sibling::thead]">
                            <xsl:for-each select=".//p">
                                <p>
                                    <xsl:attribute name="class">
                                        <xsl:value-of select="'jrnlExmTblBody'"/>
                        ...
                        <xsl:otherwise>
                            <xsl:choose>
                                <xsl:when test=".[@data-override-style]">
                                <xsl:variable name="attribValue" select="./@data-override-style"/>
                                    <xsl:for-each select=".//p">
                                        <p>
                                            <xsl:attribute name="class">
                                ...
                                <xsl:otherwise>
                                    <xsl:for-each select=".//p">
                                        <p>
                                            <xsl:attribute name="class">
                                                <xsl:value-of select="'jrnlExmNumTblBody'"/>
                                            </xsl:attribute>
                                            <xsl:apply-templates select=" ./node() "/>
                                        </p>
                                    </xsl:for-each>
    ...
    <xsl:template match="//p[@data-class='example-stmt']" priority="5">
        <xsl:element name="{name(.)}">
          <xsl:attribute name="class">
                <xsl:value-of select="'jrnlExampleStmt'"/>
            </xsl:attribute>
            <xsl:apply-templates select=" @*[name()!='class']|./node()except(*[@class='jrnlStyledContent'])"/>
            <xsl:variable name="jrnlStyledContent" select="./*[@class='jrnlStyledContent']"/>
                <xsl:if test="./child::*[@class='jrnlStyledContent']">
                    <span>
                        <xsl:value-of select="'&#x9;'"/>
    ...
    <xsl:template match="//p[@data-class='example-stmt-sub']" priority="5">
        <xsl:element name="{name(.)}">
          <xsl:attribute name="class">
                <xsl:value-of select="'jrnlExampleStmt_Sub'"/>
            </xsl:attribute>
            <xsl:apply-templates select=" @*[name()!='class']|./node()except(*[@class='jrnlStyledContent'])"/>
            <xsl:variable name="jrnlStyledContent" select="./*[@class='jrnlStyledContent']"/>
                <xsl:if test="./child::*[@class='jrnlStyledContent']">
    ...
    <xsl:template match="//p[@data-class='example-stmt-sub-sub']" priority="5">
        <xsl:element name="{name(.)}">
          <xsl:attribute name="class">
                <xsl:value-of select="'jrnlExampleStmt_Sub_Sub'"/>
            </xsl:attribute>
            <xsl:apply-templates select=" @*[name()!='class']|./node()except(*[@class='jrnlStyledContent'])"/>
            <xsl:variable name="jrnlStyledContent" select="./*[@class='jrnlStyledContent']"/>
                <xsl:if test="./child::*[@class='jrnlStyledContent']">
	...
	<xsl:template match="//p[@class='jrnlRefText']/span[@class='RefDOI']" priority="5">
	...
	<xsl:template match="//*[@class='jrnlTblBlock'][@data-type='Example']//table" priority="6">
        <xsl:element name="{name(.)}">
            <xsl:attribute name="data-class">
                <xsl:value-of select="'jrnlExampleTbl'"/>
            </xsl:attribute>
            <xsl:apply-templates select=" @*|node()"/>
        </xsl:element>
    </xsl:template>
  ...
  <!-- <xsl:template match="div[@type='main']/div[@class='floatBlock'][not(./*[@class='jrnlFootNoteFN'])]" priority="5"/> -->
   <!-- <xsl:template match="//div[@type='main']/div[@class='floatBlock'][not(@data-page-num|@data-float-placement)][not(./*[@class='jrnlFootNoteFN'])]" priority="5"/-->
    <xsl:template match="//span[@class='jrnlFirstCitation']" priority="5"/>
    <xsl:template match="//h1[@class='jrnlAbsHead']" priority="5"/>
    <xsl:template match="//div[@class='back']/*[@class='jrnlFootNoteFN']" priority="6"/>
    <!--<xsl:choose>
